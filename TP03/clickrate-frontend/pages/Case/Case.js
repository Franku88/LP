  ...
  async function mostrarCaja(crateId) {
    try {
      ...
      const [crates, allSkins] = await Promise.all([
        fetch(API_URL+"/crates").then(res => res.json()),
        fetch(API_URL+"/skins").then(res => res.json())
      ]);
      ...
      caseDiv.innerHTML = `
        ...
        <div class="crate-info">
          <div class="crate-image-wrapper container_selected" data-id="${crate.id}">
            <img src="${API_URL+"/images/crate/"+crate.id || ''}" alt="${crate.name}">
      ...
      function comprarCaja(crate) {
        ...
        if (existe) {//si ya existe, incremento el contador de la caja
          ...
          const nuevaCaja = {
            id: crate.id,
            name: crate.name,
            image: API_URL+"/images/crate/"+crate.id,
            cantidad: 1,
          };
      ...
      crate.contains.forEach(item => {
        ...
        if (skinData) {
          ...
          skinDiv.innerHTML = `
          <a href="../Skin/Skin.html?id=${skinData.id}">
            <img src="${API_URL+"/images/skin/"+skinData.id}" alt="${skinData.name}">
            <h3>${skinData.name}</h3>
          </a>
          `;
